...
public:
    ...
    // Delete from list L1 nodes occupying positions indicated in ordered lists
    // L2 and L3.
...
template <typename T>
ostream& operator<<(ostream& os, const List<T>& l) {
    if (l.Empty()) return os << "empty list";
...
template <typename T>
List<T>::List(const initializer_list<T>& l)
    : size_{0}, head_{nullptr}, tail_{nullptr} {
    for (auto& i : l) this->PushBack(i);
}
...
T& List<T>::Front() {
    ...
    throw "List::Front on an empty list!";
}
...
T& List<T>::Back() {
    ...
    if (tail_) return tail_->value;

    throw "List::Back on an empty list!";
}
